...
// ── PROJECTILE & KNOCKBACK CONSTANTS ─────────────────────────────────
export const PROJ_SPEED = 450;                  // 투사체 이동 속도 (픽셀/초)
export const INV_PROJ_SPEED = 1 / PROJ_SPEED;   // 투사체 역수 속도 (나눗셈 최적화용)
export const BASE_KNOCKBACK_IMPULSE = 170;       // 일반 몬스터 피격 시 기본 넉백 임펄스 수치
...
export const SD: Record<string, SkillDef> = {
  POWER: {
    ...
    col: '#e53935',
    lvls: [
      { d: '모든 피해 +20%', fx: (P) => { P.atkM += 0.20; } },
      { d: '모든 피해 +20% (총 +40%)', fx: (P) => { P.atkM += 0.20; } },
      { d: '모든 피해 +20% (총 +60%)', fx: (P) => { P.atkM += 0.20; } },
      { d: '모든 피해 +20% (총 +80%)', fx: (P) => { P.atkM += 0.20; } },
      { d: '💪 모든 피해 +60%p (총 ×2.4)', fx: (P) => { P.atkM += 0.60; } },
    ]
  },
  SPREAD: {
    nm: '스프레드 샷',
    ...
    lvls: [
      ...
      { d: '공격 속도 +25%', fx: (P) => { P.atkCd *= 0.80; } },
      { d: '투사체 +1 (총 4발)', fx: (P) => { P.pc = 4; } },
      { d: '💪 발사 수 2배 (총 8발)', fx: (P) => { P.pc = 8; } },
      { d: '⭐ 각성: 12발 & 공속 +20% / 황금 대형탄', fx: (P) => { P.pc = 12; P.atkCd *= 0.80; P.awk.SPREAD = true; } },
    ]
  },
  ORBS: {
    nm: '매직 오브',
  ...
  SPEED: {
    ...
    col: '#0288d1',
    lvls: [
      { d: '이동 속도 +20%', fx: (P) => { P.spdM += 0.20; } },
      { d: '이동 속도 +20% (총 +40%)', fx: (P) => { P.spdM += 0.20; } },
      { d: '이속 +20% & 자석 범위 +60%', fx: (P) => { P.spdM += 0.20; P.magR += 60; } },
      { d: '이동 속도 +10% (총 +70%)', fx: (P) => { P.spdM += 0.10; } },
      { d: '💪 자석 2배 (320) & 이속 +25%p', fx: (P) => { P.spdM += 0.25; P.magR += 160; } },
    ]
  },
  METEOR: {
    nm: '메테오',
