...
export default function App() {
  ...
  const drawTitleCanvas = (ctx: CanvasRenderingContext2D, ts: number) => {
    ...
    const anim = ts / 1000;
    for (let i = 0; i < 7; i++) {
      const a = anim * 0.35 + i * (Math.PI * 2 / 7);
      const ox = LW / 2 + Math.cos(a) * 178;
      const oy = LH / 2 + Math.sin(a) * 96;
      ctx.fillStyle = `rgba(255,213,79,${0.035 + Math.sin(a * 2) * 0.018})`;
      ctx.beginPath(); ctx.arc(ox, oy, 88, 0, Math.PI * 2); ctx.fill();
    }
    ...
    ctx.textAlign = 'center';
    ctx.fillText('🍁 SURVIVOR', LW / 2, LH / 2 - 42);
    ctx.fillStyle = '#fff9c4';
    ctx.font = "bold 78px 'Segoe UI', sans-serif";
    ctx.fillText('RUSH', LW / 2, LH / 2 + 42);
    ctx.restore();

    ctx.fillStyle = '#90a4ae';
    ctx.font = "15px 'Segoe UI', sans-serif";
    ctx.textAlign = 'center';
    ctx.fillText('MapleStory × Vampire Survivors', LW / 2, LH / 2 + 72);

    const best = (() => { try { return JSON.parse(localStorage.getItem('sr_best') || 'null'); } catch(e) { return null; } })();
    if (best) {
      ctx.fillStyle = '#ffd54f';
      ctx.font = "bold 14px 'Segoe UI', sans-serif";
      ctx.fillText(`${t('title_best')}: ${best.grade} · ${best.score.toLocaleString()}`, LW / 2, LH / 2 + 92);
    }

    // Start button
    const bw = 232, bh = 56, bx = LW / 2 - 116, by = LH / 2 + 106;
    const hov = mouseRef.current.mx >= bx && mouseRef.current.mx <= bx + bw && mouseRef.current.my >= by && mouseRef.current.my <= by + bh;

    ctx.save();
    ctx.shadowColor = hov ? '#ffd54f' : 'transparent';
    ctx.shadowBlur = hov ? 20 : 0;
    ctx.fillStyle = hov ? '#ffd54f' : '#ffb300';
    ctx.beginPath();
    ...
    ctx.textAlign = 'center';
    ctx.fillText(t('title_start'), LW / 2, by + 36);
    ...
    const lbw = 38, lbh = 28, lgap = 8;
    const ltw = langs.length * lbw + (langs.length - 1) * lgap;
    let lx = LW / 2 - ltw / 2;
    const ly = by + bh + 8;
    langs.forEach(({ code, label }) => {
      const lhov = mouseRef.current.mx >= lx && mouseRef.current.mx <= lx + lbw && mouseRef.current.my >= ly && mouseRef.current.my <= ly + lbh;
      ctx.fillStyle = code === lang ? '#ffd54f' : 'rgba(255,255,255,0.08)';
      ctx.beginPath(); (ctx as any).roundRect(lx, ly, lbw, lbh, 6); ctx.fill();
      ctx.strokeStyle = lhov ? '#ffd54f' : 'rgba(255,255,255,0.2)'; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = code === lang ? '#1a1a2e' : '#fff';
      ctx.font = "bold 13px 'Segoe UI', sans-serif"; ctx.textAlign = 'center';
      ctx.fillText(label, lx + lbw / 2, ly + lbh / 2 + 5);
      if (lhov && mouseRef.current.clicked) {
        setLang(code);
        setLangState(code);
        mouseRef.current.clicked = false;
      }
    ...
    // 타이틀 페이즈에서는 호버 정밀도와 상관없이 버튼 또는 화면 어디든 터치/클릭하면 즉각 게임을 시작할 수 있도록 연동합니다.
    if (mouseRef.current.clicked) {
      engine.resetGameState();
      engine.setPhase('skillpick');
      mouseRef.current.clicked = false;
    ...
    ctx.beginPath();
    (ctx as any).roundRect(LW / 2 - 270, LH / 2 + 204, 540, 70, 8);
    ctx.fill();

    ctx.font = "12px 'Segoe UI', sans-serif";
    ctx.textAlign = 'center';
    ctx.fillStyle = '#b0bec5';
    ctx.fillText(t('title_controls'), LW / 2, LH / 2 + 228);

    ctx.font = "11px 'Segoe UI', sans-serif";
    ctx.fillStyle = 'rgba(255,255,255,0.22)';
    ctx.fillText(t('title_mobile'), LW / 2, LH / 2 + 248);
  };
